 ...
 * @newNode: A double pointer to store the newNode node.
 * @value: The value to insert into the AVL tree.
 *
 * Return: A pointer to the newNode root after insertion, or NULL on failure.
 */
avl_t *avl_insert_recursive(avl_t **tree, avl_t *parent,
		avl_t **newNode, int value)
{
	...
	if ((*tree)->n > value)
	{
		(*tree)->left = avl_insert_recursive(&(*tree)->left, *tree, newNode, value);
		if ((*tree)->left == NULL)
			return (NULL);
	}
	else if ((*tree)->n < value)
	{
		(*tree)->right = avl_insert_recursive(&(*tree)->right, *tree, newNode, value);
		if ((*tree)->right == NULL)
			return (NULL);
	}
	else
